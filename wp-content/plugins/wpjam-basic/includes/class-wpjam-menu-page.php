...
class WPJAM_Plugin_Page{
	...
	public function tab_page(){
		...
		if($tab_object = $this->current_tab){
			if($tab_count == 1 && $this->summary && is_null($tab_object->summary)){
				$tab_object->summary	= $this->summary;
			}

			$tab_object->title(true, $tab_count);
			$tab_object->page();
		}
	}

	public function title($doing_tab=false, $tab_count=0){
		$page_title	= $this->page_title ?? $this->title;

		if($page_title){
			$subtitle	= $this->subtitle;

			if($doing_tab && $tab_count > 1){
