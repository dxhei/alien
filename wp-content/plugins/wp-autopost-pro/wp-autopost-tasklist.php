...
<?php 
global $t_ap_config,$t_ap_config_option,$t_ap_config_url_list,$t_ap_updated_record,$t_ap_log,$t_ap_more_content,$t_ap_proxy;?>
<?php 
@$id = $_REQUEST['id'];
?>
...
<div class="wrap">
<div class="icon32" id="icon-wp-autopost"><br/></div>
<h2>Auto Post - New Group</h2>



<?php
if($saction=='DeleteGroup'){
  $wpdb->query( $wpdb->prepare("update $t_ap_config set group_id=0 where group_id=%d",$_POST['group_id']));
  $wpdb->query( $wpdb->prepare("delete from $t_ap_config_group  where id=%d",$_POST['group_id']));
}

if($saction== 'saveNewGroup'){  
  $group_name = $_POST['group_name'];
  if($group_name!=null&&$group_name!=''){ 
	$wpdb->query( $wpdb->prepare("insert into $t_ap_config_group(group_name) values ( %s )",$group_name));  
    $_REQUEST['p'] = 1;

  }
  echo '<div id="message" class="updated fade"><p>'.__('A new group has been created.','wp-autopost').'</p></div>';
 }
?>


<form id="myform"  method="post" action="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php" > 
<input type="hidden" name="saction" id="saction" value="saveNewGroup">
<input type="hidden" name="group_id" id="group_id" value="">
  <br/> 
  <table> 	   
       <tbody id="the-list">         	  
       <tr> 
		 <td width="10%"><?php echo __('Group Name','wp-autopost'); ?>:</td>
		 <td><input type="text" name="group_name" id="new_group_name" value="" size="60"> <input type="button" class="button-primary" value="<?php echo __('Submit'); ?>"  onclick="addNewGroup()"/></td>
		 <td></td>
	   </tr>
	   </tbody>
  </table> 
  <br/><br/>
  <?php $groups = $wpdb->get_results("select * from $t_ap_config_group order by id"); ?>
   <?php if($groups!=null){ ?>
	<table class="widefat plugins"  > 
	   <thead>
	   <tr>
	    <th scope="col" style="text-align:center"><?php echo __('Group','wp-autopost'); ?></th>
	   ...
	   <tr> 
		 <td  style="text-align:center"><strong><?php echo $group->group_name; ?></strong></td>
		 <td style="text-align:center" > <span class="trash"><a class="submitdelete delete" title="delete" href="javascript:;" onclick="DeleteGroup(<?php echo $group->id; ?>)" ><?php echo __('Delete'); ?></a></span> </td>
	   </tr>
	   <?php } ?>
	   </tbody>
     </table>
	 <?php } ?>
  <p class="submit"> <a href="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php" class="button"><?php echo __('Return','wp-autopost'); ?></a></p>
</form>
...
<?php break;
 ...
 if($saction=='newConfig'){
  ...
  if($copy_task_id==0){
	...
	$wpdb->query( $wpdb->prepare("insert into $t_ap_config(name) values ( %s )",$config_name));
...
if(isset($_POST['saction2'])&& $_POST['saction2']=='autoseturl'){
 ...
 if($_POST['source_type']==0 || $_POST['source_type']==2){ 
  ...
  foreach($urls as $url){
    $url=trim($url);
	if($url!=''){
	  //$wpdb->query('insert into '.$t_ap_config_url_list.'(config_id,url) values ('.$id.',"'.$url.'")');
      $wpdb->query($wpdb->prepare("insert into $t_ap_config_url_list(config_id,url) values (%d,%s)",$id,$url));
	}
...
if( @$config->source_type==1 ){
  echo '<div class="updated fade"><p>'.__('Use <strong>Batch generate The URL of Article List</strong>, will not automatically update, please use manual update (Manually Click the Update Now)','wp-autopost').'</p></div>';
}
...
<h2 class="nav-tab-wrapper">
  <a class="nav-tab" href='javascript:;' id="tab-title-tab1"><?php echo __("Basic Settings","wp-autopost"); ?></a>
  <a class="nav-tab" href='javascript:;' id="tab-title-tab2"><?php echo __("Extraction Settings","wp-autopost"); ?><!-- 采集设置 --></a>
  <a class="nav-tab" href='javascript:;' id="tab-title-tab3"><?php echo __("WordPress Settings","wp-autopost"); ?><!-- WordPress相关 --></a>
  <a class="nav-tab" href='javascript:;' id="tab-title-tab4"><?php echo __("Filter & Replacement","wp-autopost"); ?><!--数据处理--></a>
  <a class="nav-tab" href='javascript:;' id="tab-title-tab5"><?php echo __("Custom Settings","wp-autopost"); ?><!--自定义样式及模板--></a>
  <a class="nav-tab" href='javascript:;' id="tab-title-tab6"><?php echo __("Rewrite & Translate","wp-autopost"); ?><!--伪原创及翻译--></a>																			
</h2>

<div id="tab-tab1" class="div-tab hidden"> <!-- 基本设置 -->
<br/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group" value="0">
...
<div class="postbox">
  ...
  <div class="inside" <?php //if(@!$showBox1)echo 'style="display:none;"'; ?> >
	 <table width="100%"> 	         	  
	   ...
	   <tr> 
		 <td width="18%"><?php echo __('Group','wp-autopost'); ?>:</td>
		 <td>
		   <?php $groups = $wpdb->get_results("select * from $t_ap_config_group order by id"); ?>
		   
		   <select name="group_id" id="group_id">
             <option value="0" <?php if($config->group_id=='0') echo 'selected="true"'; ?>><?php echo __('Not in group','wp-autopost'); ?></option>
             <?php foreach($groups as $group) { ?>
               <option value="<?php echo $group->id; ?>" <?php if( ($config->group_id)==($group->id)) echo 'selected="true"'; ?> ><?php echo $group->group_name; ?></option>
			 <?php } ?>
		   </select>
	   ...
	   <tr> 
        ...
        <td>
		 <?php $proxys = $wpdb->get_results('SELECT id,proxy_name FROM '.$t_ap_proxy.' order by id'); ?> 
		 <select id="use_proxy" name="use_proxy"> 
           <option value="0" <?php if($proxy[0]==0) echo 'selected="true"'; ?>><?php echo __('No'); ?></option>
		    <?php foreach($proxys as $the_proxy){ ?>
                 <option value="<?php echo $the_proxy->id; ?>" <?php selected( $the_proxy->id, $proxy[0]); ?> > <?php echo __('Proxy Name','wp-autopost'); ?> : <?php echo $the_proxy->proxy_name; ?></option>
             <?php } ?>
		   <!-- <option value="1" <?php if($proxy[0]==1) echo 'selected="true"'; ?>><?php echo __('Yes'); ?></option> -->
		 </select>
		  (<a href="admin.php?page=wp-autopost-pro/wp-autopost-proxy.php" target="_blank"><?php echo __('Proxy Options','wp-autopost'); ?></a>)
		</td>
	   </tr>
	   ...
	   <tr>
         <td colspan="2">
		   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"   onclick="edit(0)"/>
		   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="edit(1)"/>
		 </td>
	   </tr>
    </table>
  </div>
...
<div id="tab-tab2" class="div-tab hidden"> <!-- 采集设置 -->
<br/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group2" value="0">


<?php $urls = $wpdb->get_results('SELECT * FROM '.$t_ap_config_url_list.' WHERE config_id ='.$id.' ORDER BY id' ); ?>
<div class="postbox"> <!-- Article Source Settings -->
	   ...
	   <tr> 
		 <td>	 
		   ...
		   <div id="urlArea2" <?php if(($config->source_type)!=1) echo 'style="display:none;"'; ?>>
             ...
             <?php
               echo '<p><code>'.__('Use <strong>Batch generate The URL of Article List</strong>, will not automatically update, please use manual update (Manually Click the Update Now)','wp-autopost').'</code></p>';
...
jQuery(document).ready(function($){   
  ...
  <?php } ?>
	...
	?>
	  ...
	  <br/>
	  <br/>
	  <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save2(0)"/>
	  <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save2(1)"/>
	  <input type="button" class="button"  value="<?php echo __('Test','wp-autopost'); ?>"  onclick="test2()"/>

  </div><!-- end class="inside"  -->
</div><!-- end Article Source Settings -->
<div class="clear"></div>
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group3" value="0">

<div class="postbox"> <!-- Article Extraction Settings -->
...
?>    
  ...
  <div style="margin-bottom:12px;">
	...
	<div id="autosetenterurl" style="margin-top:5px;display:none;">
       <?php echo __('Enter a URL of article','wp-autopost'); ?>:<input type="text" name="autoset_url" id="autoset_url" value="<?php if(isset($_POST['autoset_url']))echo $_POST['autoset_url']; ?>" size="100" />
...
<?php }//end if($cmrNum>1) ?>
  ...
  <table>
   <tr><td>  
	 ...
	 <span class="gray"><code><?php echo __('(*) is Wildcards','wp-autopost'); ?></code></span>
	 <br/><br/>
	 <input type="checkbox" name="force_check_page_pattern" id="force_check_page_pattern" <?php if($page_selector[3]==1)echo 'checked="true"'; ?> /> <?php echo __('Force to check the Paginated URL Pattern','wp-autopost'); ?>
     <a title='<?php echo __('If you find the extracted Paginated Content is not right, try select this option','wp-autopost'); ?>'>[?]</a>
	 &nbsp;
	 <input type="checkbox" name="force_check_page_validity" id="force_check_page_validity" <?php if($page_selector[2]==1)echo 'checked="true"'; ?> /> <?php echo __('Force to check the paginated validity','wp-autopost'); ?>
     <a title='<?php echo __('If you find the extracted Paginated Content is not right, try select this option','wp-autopost'); ?>'>[?]</a>
	...
	<div id="page_match_1" <?php if($page_selector[0]!=1) echo 'style="display:none;"'; ?>> 
	   ...
	   <?php
		  //兼容前面版本
	      if(strpos($page_selector[1],'WPAPSPLIT')===false){
             $match = explode('(*)',trim($page_selector[1]));
		  }else{
             $match = explode('WPAPSPLIT',trim($page_selector[1]));
		  }
		...
		<table>
          <tr> 
            <td><?php echo __('Starting Unique HTML','wp-autopost'); ?>:</td>
			<td>
			  <input type="text" name="page_selector_1_start" id="page_selector_1_start"  class="rss_disable content_selector" size="40" value="<?php if($page_selector[0]==1) echo htmlspecialchars($match[0]); ?>">
			  <span class="gray"> 
                <?php echo __('For example','wp-autopost'); ?>: <code>&ltdiv id="page-(*)"></code>  OR <code>&ltdiv id="paged"></code>
			  </span>
			</td>
		  </tr>
		  <tr>
            <td><?php echo __('Ending Unique HTML','wp-autopost'); ?>:</td>
			<td>
			  <input type="text" name="page_selector_1_end" id="page_selector_1_end"class="rss_disable content_selector" size="40" value="<?php if($page_selector[0]==1) echo htmlspecialchars($match[1]); ?>">
			  <span class="gray"> 
                <?php echo __('For example','wp-autopost'); ?>: <code>&lt/div></code>
			  </span>
			</td>
		  </tr>
		</table>
...
?> 
  ...
  <div>
    <br/>
	<input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>" <?php if(($config->auto_set)!= '' && ($config->auto_set)!= null){ echo 'disabled="true"'; }  ?> onclick="save3(0)"/>

	<input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>" <?php if(($config->auto_set)!= '' && ($config->auto_set)!= null){ echo 'disabled="true"'; }  ?> onclick="save3(1)"/>
 ...
 </div><!-- end div class="inside" -->
</div><!-- end Article Extraction Settings -->
<div class="clear"></div>
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group15" value="0">

<div class="postbox"><!--  Site Login Settings  -->
  <h3 class="hndle" style="cursor:pointer;">
    <span class="TipRss" <?php if(($config->source_type)!=2) echo 'style="display:none"'; ?> > 
      <?php echo __('Use RSS do not need this setting','wp-autopost'); ?>	 
    </span>
  ...
  <div class="inside" <?php if(@!$showBox15)echo 'style="display:none;"' ?> >
	...
	<p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save15(0)"/>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save15(1)"/>
	...
	<div id="testCookie" style="display:none;" >
      <?php echo __('Enter the URL of test extraction','wp-autopost'); ?>:<input type="text" name="testcCookieUrl" id="testcCookieUrl" value="<?php if(isset($_POST['testcCookieUrl']))echo $_POST['testcCookieUrl']; ?>" size="100" />
      <input type="button" class="button-primary"  value="<?php echo __('Submit'); ?>"  onclick="testCookie()"/>
    </div>

  </div><!-- end div class="inside" -->
</div><!-- end Site Login Settings -->
</form>


</div> <!-- end tab-tab2 -->

<div id="tab-tab3" class="div-tab hidden"> <!-- WordPress相关 -->
<br/>

<form id="myform18"  method="post" action="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php">
<input type="hidden" name="saction" value="save18">
<input type="hidden" name="saction18" id="saction18" value="">
<input type="hidden" name="id"  value="<?php echo $id; ?>">
<input type="hidden" name="p"  value="<?php echo $_REQUEST['p']; ?>">

<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group18" value="0">

<div class="postbox"> <!-- General Settings -->
  <h3 class="hndle" style="cursor:pointer;">
	<?php echo __('General Settings','wp-autopost'); ?>
  </h3>
  <div class="inside" <?php if(@!$showBox18)echo 'style="display:none;"' ?> >

    <table width="100%"> 	         	  
     <tr> 
		 <td width="18%" style="padding:10px 0 10px 0;"><?php echo __('Post Type','wp-autopost'); ?>:</td>
         <td style="padding:10px 0 10px 0;">
...
<?php endif; ?>   
<?php endif; // end if ( current_theme_supports( 'post-formats' ) ): ?>
	   ...
	   <?php
		  ...
		  if(!is_array($auto_set)){
             ...
             $auto_set[4] = 0; // limit_tags_number
	   ...
	   <tr>
	     <td colspan="2"><p><strong><?php echo __('Publish Date Setting','wp-autopost'); ?></strong></p></td>
	   </tr>
	   ...
	   <tr>
        ...
        <td>
           <?php 
		     $published_interval = $config->published_interval;
	         if($post_scheduled[0]==2){ $published_interval=0; $published_interval_attr = 'disabled="true"'; }  
		   
		   ?>
		   <input type="text" name="published_interval" id="published_interval" size="2" value="<?php echo $published_interval; ?>" <?php echo $published_interval_attr; ?> > <?php echo __('Minute','wp-autopost'); ?> <span class="gray">( <?php echo __('The published date interval between each post','wp-autopost'); ?> )</span>
		</td> 
	   </tr>

	   <tr>
        <td><?php echo __('Post Instant','wp-autopost'); ?>:</td>
        <td>
		  <select id="post_scheduled1" name="post_scheduled1">
           <option value="0" <?php if($post_scheduled[0]==0) echo 'selected="true"'; ?>><?php echo __('No'); ?></option>
		   <option value="2" <?php if($post_scheduled[0]==2) echo 'selected="true"'; ?>><?php echo __('Yes'); ?></option>
		  </select>
       ...
       <tr>
         <td></td>
		 <td>
           <div id="post_scheduled_more" <?php if($post_scheduled[0]==0||$post_scheduled[0]==2)echo 'style="display:none;"' ?> >
	        <table>
              <tr>
	   ...
	   <tr>
        <td><?php echo __('Tags Setting','wp-autopost'); ?>:</td>
        <td>
         <select id="auto_tags" name="auto_tags">
           <option value="0" <?php if(($auto_set[0])==0) echo 'selected="true"'; ?>><?php echo __('No'); ?></option>
		   <option value="1" <?php if(($auto_set[0])==1) echo 'selected="true"'; ?>><?php echo __('Auto Tags','wp-autopost'); ?></option>
		   <option value="2" <?php if(($auto_set[0])==2) echo 'selected="true"'; ?>><?php echo __('Constant Tags','wp-autopost'); ?></option>
		 </select>	 
		</td> 
	   </tr>
	   <tr>
		 ...
		 <td>
		  <span id="tags_div" <?php if($auto_set[0]==0)echo 'style="display:none;"' ?> >
		   ...
		   <p class="tag_set1" <?php if($auto_set[0]!=1)echo 'style="display:none;"' ?> ><strong><?php echo __("When article's contents contains the keyword in the below tags list, will add the matched tags",'wp-autopost'); ?></strong></p>
		   
		   <p class="tag_set2" <?php if($auto_set[0]!=2)echo 'style="display:none;"' ?> ><strong><?php echo __("Ignore article's contents, will always add the tags in the below list",'wp-autopost'); ?></strong></p>
		   ...
		   <textarea style="width:100%" name="tags" id="tags" ><?php echo $config->tags; ?></textarea>		   
		    <br/>
		   <span class="tag_set1" <?php if($auto_set[0]!=1)echo 'style="display:none;"' ?> >
		   ...
		   <br/>	   
		   <input type="checkbox" name="whole_word" id="whole_word" <?php if(($config->whole_word)==1)echo 'checked="true"'; ?> /> <?php echo __('Match Whole Word','wp-autopost'); ?> <span class="gray">(<?php echo __('Autotag only a post when terms finded in the content are a the same name','wp-autopost'); ?>)</span>
		   <br/>
		   </span>
		   
		   <?php echo __("Limit the number of tags",'wp-autopost'); ?>: <input type="text" size="1" name="limit_tags_number" value="<?php echo $auto_set[4];?>" /> <span class="gray">( <?php echo __("0 means no limits",'wp-autopost'); ?> )</span>
		   </p> 
		 </span></td>
       </tr>
	...
	<p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save18(0)"/>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save18(1)"/>
	</p>
...
<form id="myform19"  method="post" action="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php">
<input type="hidden" name="saction" value="save19">
<input type="hidden" name="saction19" id="saction19" value="">
<input type="hidden" name="id"  value="<?php echo $id; ?>">
<input type="hidden" name="p"  value="<?php echo $_REQUEST['p']; ?>">

<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group19" value="0">

<div class="postbox"> <!--Images Settings -->
  <h3 class="hndle" style="cursor:pointer;">
	<?php echo __('Images Settings','wp-autopost'); ?>
  </h3>
  <div class="inside" <?php if(@!$showBox19)echo 'style="display:none;"' ?> >
	 ...
	 <?php
		   $img_insert_attachment = json_decode($config->img_insert_attachment);    
			 if(!is_array($img_insert_attachment)){
			    ...
			    $img_insert_attachment[5] = 0; // download a tag‘s images
			    $img_insert_attachment[6] = 0; // downImgFailsNotPost
			    $img_insert_attachment[7] = 0; // wp_autopost_downImgThumbnail
			    $img_insert_attachment[8] = 1000; // wp_autopost_downImgMaxWidth
			    $img_insert_attachment[9] = 90; // wp_autopost_downImgQuality
			    $img_insert_attachment[10] = 100;// wp_autopost_downImgMinWidth
			 }
		     
	 ?>
	   
	   <?php  if( ($img_insert_attachment[4]===null)||($img_insert_attachment[4]===0)){$attribute='src';}else{$attribute = $img_insert_attachment[4];} ?> 
	   <tr>
	    <td width="18%"> 
		  <?php echo __('The attribute of image\'s URL','wp-autopost'); ?>:
		</td>
	    <td> 
	      <input type="text" name="img_url_attr" value="<?php echo $attribute; ?>" /> <span class="gray">( <?php echo __('Usually is " src "','wp-autopost'); ?> )</span>
	    </td>
	   </tr>
	   
	   <tr>  
        <td width="18%"><?php echo __('Download Remote Images','wp-autopost'); ?>:</td>
        <td>
	   ...
	   <tr>
         ...
         <span id="img_insert_attachment_div" <?php if($download_attachs[0]==0)echo 'style="display:none;"' ?> >		  
             ...
             <table>
               <tr>
                 <td><?php echo __('Also Download Images Form A Tags','wp-autopost'); ?> : </td>
                 <td> 
                   <?php if(!isset($img_insert_attachment[5]))$img_insert_attachment[5]=0; ?>      
		           <select name="down_img_from_a" >		       		        
		            <option value="0" <?php selected( 0, $img_insert_attachment[5]); ?> ><?php echo __('No'); ?></option>
		            <option value="1" <?php selected( 1, $img_insert_attachment[5]); ?> ><?php echo __('Yes'); ?></option>
		           </select>
		           <span class="gray">( <?php echo __('Like: &lt;a <code>href="big_one.jpg"</code>>&lt;img src="small_one.jpg" />&lt;/a>','wp-autopost'); ?> )</span> 
                 </td>
               </tr>       
               <tr>
               ...
               <tr>
                 <td><?php echo __('When download images fails will not post','wp-autopost'); ?> : </td>
                 <td>
                  <?php if(!isset($img_insert_attachment[6]))$img_insert_attachment[6]=0; ?>
		          <select name="downImgFailsNotPost">
                    <option value="0" <?php selected( 0, $img_insert_attachment[6]); ?> ><?php echo __('No'); ?></option>
			        <option value="1" <?php selected( 1, $img_insert_attachment[6]); ?> ><?php echo __('Yes'); ?></option>
		          </select>
                 </td>
               </tr>
               
               <tr>
                 <td><?php echo __('All downloaded images generate thumbnail','wp-autopost'); ?> : </td>
                 <td>
                  <?php if(!isset($img_insert_attachment[7]))$img_insert_attachment[7]=0; ?>
		          <select name="downImgThumbnail">
                   <option value="0" <?php selected( 0, $img_insert_attachment[7]); ?> ><?php echo __('No'); ?></option>
			       <option value="1" <?php selected( 1, $img_insert_attachment[7]); ?> ><?php echo __('Yes'); ?></option>
		          </select>
               ...
               <tr>
                <td><?php echo __('Downloaded Images Size Optimization','wp-autopost'); ?> :</td>
                <td>
                  <?php if(!isset($img_insert_attachment[8]))$img_insert_attachment[8]=1000; ?>
                  <?php if(!isset($img_insert_attachment[9]))$img_insert_attachment[9]=90; ?>
                  <table>
                   <tr>
                    <td> <?php echo __('Max Width','wp-autopost'); ?></td>
			        <td>
			         <input type="text" name="downImgMaxWidth" id="downImgMaxWidth" value="<?php echo $img_insert_attachment[8]; ?>" size="2" /> PX
                   ...
                   <tr>
			        ...
			        <td>
			         <input type="text" name="downImgQuality" id="downImgQuality" value="<?php echo $img_insert_attachment[9]; ?>" size="2" /> 
               ...
               <tr>
                 <td><?php echo __('Min Width Image to Download','wp-autopost'); ?> : </td>
                 <?php if(!isset($img_insert_attachment[10]))$img_insert_attachment[10]=100; ?>
                 <td>
                  <input type="text" name="imgMinWidth" id="imgMinWidth" value="<?php echo $img_insert_attachment[10]; ?>" size="2" />  px
                 </td>
               </tr>                   
             </table>
             <br/>
	   ...
	   <tr>
	     <td><?php echo __('Add a watermark to downloaded images automatically','wp-autopost'); ?>:</td>
	     <td>  	        
			 ...
			 <select name="watermark_id" id="watermark_id">
			  ...
			  <?php foreach($wms as $wm){ ?>
			    <option value="<?php echo $wm->id; ?>" <?php selected( $wm->id, $img_insert_attachment[2]); ?> > <?php echo __('Watermark Name','wp-autopost'); ?> : <?php echo $wm->name; ?></option>
			  <?php } ?>
			 </select>
	         (<a href="admin.php?page=wp-autopost-pro/wp-autopost-watermark.php" target="_blank"><?php echo __('Watermark Options','wp-autopost'); ?></a>)
	     </td>
	   </tr>
   ...
   <p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save19(0)"/>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save19(1)"/>
   </p>
  
  
  </div><!-- end class="inside" -->
</div><!-- end Images Settings -->
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group17" value="0">
...
?>
<div class="postbox"> <!-- Default Featured Image Settings -->
  <h3 class="hndle" style="cursor:pointer;">
  ...
  <div class="inside" <?php if(@!$showBox17)echo 'style="display:none;"' ?> >
    ...
    <p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save17(0)"/>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save17(1)"/>
	</p>

  </div><!-- end div class="inside" -->
</div><!-- end Default Featured Image Settings -->
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group12" value="0">

<div class="postbox"><!-- Custom Fields -->
 ...
 <div class="inside" <?php if(@!$showBox12)echo 'style="display:none;"' ?> >
  ...
  if($config->custom_field!=null&&$config->custom_field!=''){
       $custom_field = array();
	   $old_custom_field = json_decode($config->custom_field);
...
<?php endif; ?>
  ...
  <p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"  onclick="save12(1)"/>
  </p>
...
<div id="tab-tab4" class="div-tab hidden"> <!-- 数据处理 -->
<br/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group6" value="0">

<div class="postbox"><!-- Article Content Filtering -->
...
<?php } ?> 
	...
	<p>
    <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption5(0)"/>
	<input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption5(1)"/>
...
<?php } ?> 
	...
	<p>
    <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption1(0)"/>
    <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption1(1)"/>
...
</div><!-- end Article Content Filtering -->
<div class="clear"></div>
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group7" value="0">

<div class="postbox"><!-- HTML tags Filtering -->
  ...
  <?php } ?> 
   ...
   <p>
   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption2(0)"/>
   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption2(1)"/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group8" value="0">

<div class="postbox"><!-- Article Content Keywords Replacement -->
  ...
  <div class="inside" <?php if(@!$showBox6)echo 'style="display:none;"' ?> >
   ...
   <p>
   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption3(0)"/>
   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption3(1)"/>
...
</div><!-- end Article Content Keywords Replacement -->
<div class="clear"></div>
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group9" value="0">

<div class="postbox"><!-- Article Title Keywords Replacement -->
  ...
  <div class="inside" <?php if(@!$showBox7)echo 'style="display:none;"' ?> >
   ...
   <p>
   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption4(0)"/>
   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption4(1)"/>
...
</div><!-- end Article Title Keywords Replacement -->
<div class="clear"></div>
</form>
...
<div id="tab-tab5" class="div-tab hidden" ><!-- 自定义样式及模板 -->
<br/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group14" value="0">

<div class="postbox"><!-- Custom Article Style -->
  ...
  <div class="inside" <?php if(@!$showBox14)echo 'style="display:none;"' ?> >
   ...
   <table  id="OptionType14"  class="tdCenter"  > <!-- class="autoposttable" -->
	...
	?>
     <tr id="type14<?php echo $num; ?>">
       <td>
         <input type="text" name="type14_para1[]" value="<?php echo htmlspecialchars($customStylePara[0]); ?>" >
	   </td>
	   ...
	   <td>
	    <input type="text" name="type14_para3[]" size="8" value="<?php echo htmlspecialchars($customStylePara[2]); ?>" >

	   </td>
       
	   <td>
	     <input type="text" name="type14_para4[]" size="60" value="<?php echo htmlspecialchars($customStylePara[3]); ?>" >
	   </td>
   ...
   <p>
   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption14(0)"/>
   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption14(1)"/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group10" value="0">

<div class="postbox"><!-- Insert Content In Anywhere -->
...
<?php $insertContents = $wpdb->get_results('SELECT * FROM '.$t_ap_more_content.' WHERE config_id ='.$id.' AND option_type=0 ORDER BY id' ); ?>
 ...
 <div class="inside" <?php if(@!$showBox9)echo 'style="display:none;"' ?> >
  ...
  <table  id="OptionType6" width="100%" class="autoposttable">
	...
	?>
     <tr id="type6<?php echo $num; ?>">
	  ...
	  <input type="text" name="type6_para1[]" value="<?php echo htmlspecialchars($insertContentPara[0]); ?>" >&nbsp;&nbsp;&nbsp;
  ...
  <p>
   <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveOption6(0)"/>
   <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveOption6(1)"/> 
...
</div><!-- end Insert Content In Anywhere -->
</form>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group11" value="0">

<div class="postbox"><!-- Prefix / Suffix -->
 ...
 <div class="inside" <?php if(@!$showBox8)echo 'style="display:none;"' ?> >
  ...
  <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"    onclick="SaveConfigOption(0)"/>
  <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="SaveConfigOption(1)"/> 
 </div><!-- end div class="inside" --> 
</div><!-- end Prefix / Suffix -->
</form>
...
<div id="tab-tab6" class="div-tab hidden" ><!-- 伪原创及翻译 -->
<br/>
...
<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group16" value="0">

<div class="postbox"><!-- 中文 简体/繁体 转换 -->
  ...
  <div class="inside" <?php if(@!$showBox16)echo 'style="display:none;"' ?> >
	...
	<p>
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save16(0)"/>
	 <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="save16(1)"/> 
	</p>
...
<?php
  ...
  if($BaiduTransOptions['APP_ID']!=null&&$BaiduTransOptions['APP_ID']!=''&&$BaiduTransOptions['SEC_KEY']!=null&&$BaiduTransOptions['SEC_KEY']!=''){
...
<input type="hidden" name="saction5" id="saction5" value="">
<input type="hidden" name="id"  value="<?php echo $id; ?>">
<input type="hidden" name="p"  value="<?php echo $_REQUEST['p']; ?>">


<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group5" value="0">

<div class="postbox"><!-- Rewrite (Spinning) -->
...
<?php } ?>
   ...
   <p>
     <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save5(0)"/>
     <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="save5(1)"/>
   </p>
	
  </div><!-- end div class="inside" -->
</div><!-- end Rewrite (Spinning) -->
</form>
...
<input type="hidden" name="saction4" id="saction4" value="">
<input type="hidden" name="id"  value="<?php echo $id; ?>">
<input type="hidden" name="p"  value="<?php echo $_REQUEST['p']; ?>">


<input type="hidden" name="the_group_id"  value="<?php echo $config->group_id; ?>">
<input type="hidden" name="save_to_group"  id="save_to_group4" value="0">

<div class="postbox"><!-- Translate -->
...
<?php } ?>    
  ...
  <div id="use_translator" <?php if($use_trans[0]==0){?> style="display:none;" <?php } ?> >
	...
	<p>	
	 <input type="button" class="button-primary"  value="<?php echo __('Save Changes'); ?>"  onclick="save4(0)"/>
     <input type="button" class="button-primary"  value="<?php echo __('Save To Group','wp-autopost'); ?>"    onclick="save4(1)"/>	
	</p>
  </div><!-- end div class="inside" -->
</div><!-- end Translate -->
<div class="clear"></div>
...
<div class="wrap">
  <div class="icon32" id="icon-wp-autopost"><br/></div>
  <h2>Auto Post 
    <a href="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php&saction=new" class="add-new-h2"><?php echo __('Add New Task','wp-autopost'); ?></a> 
    <a href="admin.php?page=wp-autopost-pro/wp-autopost-tasklist.php&saction=new_group" class="add-new-h2"><?php echo __('Add New Group','wp-autopost'); ?></a>
  </h2>
   <div class="clear"></div>
<?php
...
$configs = $wpdb->get_results('SELECT t1.id,t1.group_id,t1.activation,t1.name,t1.last_update_time,t1.update_interval,t1.m_extract,t1.is_running,t1.post_id,t1.last_error,t1.updated_num,t1.source_type,t1.auto_set,t1.a_selector,t1.title_selector,t1.content_selector,t2.group_name FROM '.$t_ap_config.' t1 left join '.$t_ap_config_group.' t2 on t1.group_id=t2.id ORDER BY t1.group_id,t1.id LIMIT '.$from.','.$perPage); 


//echo $t_ap_config_group;
...
?>
       <tr style="text-align:center"  <?php if(($config->activation)==0){ ?> class="inactive" <?php }else{ ?> class="active"  <?php  } ?>> 
         ...
         <td>
          <?php if($config->group_id==0){ ?>
		      <?php //echo __('Not in group','wp-autopost'); ?> 
          <?php }else{ ?>
              <strong><?php echo $config->group_name; ?></strong>
		  <?php } ?>
		 </td>
