...
class post_img_handle_ap
{
	...
	public static function handle_upload_img_new($url, $data, $mime, $file_ext, $minWidth, $downImgMaxWidth, $downImgQuality, $relativeURL, $downFileOrganize, $post_title)
	{
		...
		{
			if (false === wpapimagefilesave ($new_file, $data))
			{
				return false;
			}
		}
...
function getPageContentbyAP($post, $d, $charset, $autoSet, $config_page_selector, $hasUTFhtml, $UTFhtml, $same_paged, $content_match_type, $content_selector, $outer, $objective, $index, $options, $baseUrl, $url, $filterAtag, $downAttach, $isTest, $useP1, $useP2, $proxy, $cookie)
{
	...
	if ($pageList != null)
	{
		...
		global $page1content;
		$page1content = $post;
		$force_check_page_validity_url = null;
		$nextPageContent .= getNextPageContent($pageList, $autoSet, $content_match_type, $content_selector, $outer, $objective, $index, $page_selector, $same_paged, $delPageArea, $options, $charset, $baseUrl, $url, $filterAtag, $downAttach, $isTest, $useP1, $useP2, $proxy, $pageURLPattern, $pageNum, $cookie, $force_check_page_validity_url);
		unset($isFetch);
		unset($page1content);
		unset($pageList);
	}
...
function getNextPageContent($pageList, $autoSet, $content_match_type, $content_selector, $outer, $objective, $index, $page_selector, $paged, $delPageArea, $options, $charset, $baseUrl, $page1URL, $filterAtag, $downAttach, $isTest, $useP1, $useP2, $proxy, $pageURLPattern, $pageNum, $cookie, $force_check_page_validity_url)
{
	global $isFetch;
	$content = '';
	if ($pageList != null) foreach($pageList as $page)
	{
		...
		$url = html_entity_decode(trim($url));
		if ($page_selector[2] == 1)
		{
			if (strlen($page1URL) === strlen($url))
			{
				continue;
			}
		}
		if ($force_check_page_validity_url != null && $force_check_page_validity_url != '')
		{
			if (stripos($url, $force_check_page_validity_url) === false)continue;
		}
		...
		if ($autoSet)
		{
			$tempContent = autoGetContents($d, $charset);
		}
		else
		{
			$hasUTFhtml = false;
			...
			for($i = 0;$i < $matchNum;$i++)
			{
				if ($content_match_type[$i] == 0)
				{
					switch ($objective[$i])
					{
						case '0': $tempContent = gTheConUseCss($d, $content_selector[$i], $options, $charset, $outer[$i], $index[$i]);
							break;
					}
				}
				else
				{
					switch ($objective[$i])
					{
						case '0': $tempContent = getContentByRule($UTFhtml, $content_selector[$i], $options, $outer[$i]);
							break;
					}
				}
			}
		}
		if ($page_selector[2] == 1 && $force_check_page_validity_url == null && $tempContent != null && $tempContent != '')
		{
			$force_check_page_validity_url = return_the_same_str_wpap($page1URL, $url);
		}
		$content .= $tempContent;
		unset($tempContent);
		if ($page_selector[0] == 0)
		...
		unset($d);
		unset($UTFhtml);
		$content .= getNextPageContent($pageList, $autoSet, $content_match_type, $content_selector, $outer, $objective, $index, $page_selector, $paged, $delPageArea, $options, $charset, $baseUrl, $page1URL, $filterAtag, $downAttach, $isTest, $useP1, $useP2, $proxy, $pageURLPattern, $pageNum, $cookie, $force_check_page_validity_url);
		break;
	}
	return $content;
}
function return_the_same_str_wpap($str1, $str2)
{
...
function wp_autopostlink_replace($content, $autolinks)
{
	...
	foreach ($autolinks as $autolink)
	{
		...
		if ($ignorecase) $case = "i";
		else $case = "";
		$ex_word = preg_quote($cleankeyword, '\'');
		if ($ignore_pre)
		{
			if ($num_1 = preg_match_all("/<pre.*?>.*?<\/pre>/is", $content, $ignore_pre)) for($i = 1;$i <= $num_1;$i++) $content = preg_replace("/<pre.*?>.*?<\/pre>/is", "%ignore_pre_$i%", $content, 1);
		}
	...
	if ($autoLinkTag[0] == 1)
	{
		...
		foreach($terms as $term)
		{
			...
			$desc = $cleankeyword;
			$desc = addcslashes($desc, '$');
			$url = '<a href=' . '"' . get_tag_link($term->term_id) . '"' . ' title=' . '"' . $desc . '"';
			if ($nofollow) $url .= ' rel=' . '"' . 'nofollow' . '"';
			if ($newwindow) $url .= ' target=' . '"' . '_blank' . '"';
			$url .= ">" . addcslashes($cleankeyword, '$') . "</a>";
			if ($firstonly) $limit = 1;
...
class autopostMicrosoftTranslator
{
	...
	public static function bulid_lang_options($selected = '')
	{
		...
		{
			$options .= '<option value="' . $key . '" ' . (($selected == $key)?'selected="true"':'') . '>' . __($value, 'wp-autopost') . '</option>';
		}
		return $options;
	}
...
function ap_baidu_translate($query, $from, $to, $APP_ID, $SEC_KEY)
{
	$args = array('q' => $query, 'appid' => $APP_ID, 'salt' => rand(10000, 99999), 'from' => $from, 'to' => $to,);
	$args['sign'] = ap_baidu_buildSign($query, $APP_ID, $args['salt'], $SEC_KEY);
	$ret = ap_baidu_call("http://api.fanyi.baidu.com/api/trans/vip/translate", $args);
	$ret = json_decode($ret, true);
	return $ret;
}
function ap_baidu_callOnce($url, $args = null, $method = "post", $withCookie = false, $timeout = CURL_TIMEOUT, $headers = array())
{
	$ch = curl_init();
	if ($method == "post")
	{
		$data = ap_baidu_convert($args);
...
function ap_baidu_convert(&$args)
{
	$data = '';
	if (is_array($args))
	{
...
{
	private static $language_code = array ('zh' => 'Chinese', 'en' => 'English', 'yue' => 'Cantonese', 'cht' => 'Traditional Chinese', 'jp' => 'Japanese', 'kor' => 'Korean', 'fra' => 'French', 'spa' => 'Spanish', 'th' => 'Thai', 'ara' => 'Arabic', 'ru' => 'Russian', 'pt' => 'Portuguese', 'de' => 'German', 'it' => 'Italian', 'el' => 'Greek', 'nl' => 'Dutch', 'pl' => 'Polish', 'bul' => 'Bulgarian', 'est' => 'Estonian', 'dan' => 'Danish', 'fin' => 'Finnish', 'cs' => 'Czech', 'rom' => 'Romanian', 'slo' => 'Slovene', 'swe' => 'Swedish', 'hu' => 'Hungarian');
	public static function bulid_lang_options($selected = '')
	{
		$options = '';
		foreach (self::$language_code as $key => $value)
...
class autopost_html_dom
{
	...
	protected function prepare($str, $lowercase = true, $stripRN = true, $defaultSNode = null, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT)
	{
		...
		if (self::$htmldomnode === 0)
		{
			...
			if ($sequence == '' || $sequence == null || $metets == null || $metets == '')
			{
...
function getRawUrl($url)
{
	if (strpos($url, '%') === false)
	{
		...
		$a = array("%3A", "%2F", "%40", "%3F", "%26", "%23", "%3D");
		$b = array(":", "/", "@", "?", "&", "#", "=");
		$url = str_replace($a, $b, $url);
	}
	return $url;
}
function ap_socks5_proxy($url, $proxy, $referer = '')
{
	$ua = get_user_agent_ap();
	if ($referer == '')
	{
		$referer = $url;
	}
	$cmd = sprintf('curl -s -A "%s" --referer "%s" --socks5 %s:%s "%s"', $ua, $referer, $proxy['ip'], $proxy['port'], $url);
	$out = shell_exec($cmd);
	return $out;
}
